...
```{r los_90_full}
los_90_full <- quantile(duke_nicu_clean$length_of_stay, 0.90, na.rm = TRUE)
```
...
ggplot(data = duke_nicu_clean,
  ...
  geom_vline(xintercept = los_90_full,
             color = "red",
             linetype = "dashed",
             linewidth = 1) +
  theme_minimal() +
...
```{r duke_nicu_clean-summary-stats}
duke_nicu_clean %>%
  summarize(n = sum(!is.na(length_of_stay)),
            mean = mean(length_of_stay, na.rm = TRUE),
            sd = sd(length_of_stay, na.rm = TRUE),
            median = median(length_of_stay, na.rm = TRUE),
            Q1 = quantile(length_of_stay, 0.25, na.rm = TRUE),
            Q3 = quantile(length_of_stay, 0.75, na.rm = TRUE),
            IQR = IQR(length_of_stay, na.rm = TRUE))
```
...
```{r duke_nicu_clean-scatter, message = FALSE, warning = FALSE}
ggplot(data = duke_nicu_clean,
       mapping = aes(x = gestational_age,
                     y = length_of_stay)) +
  ...
  labs(x = "Gestational Age (Weeks)",
       ...
       title = "Length of Stay vs. Gestational Age") +
  theme_minimal()
```
...
```{r duke_nicu_clean-scatter2, warning = FALSE}
ggplot(data = duke_nicu_clean,
       mapping = aes(x = birth_weight,
                     y = length_of_stay)) +
...
ggplot(data = duke_nicu_clean,
       mapping = aes(x = factor(race_ethnicity,
                                levels = c("White", "Black", "Other")),
                     fill = race_ethnicity)) +
  geom_bar(show.legend = FALSE) +
  scale_fill_manual(values = c("White" = "#4E79A7",
                               "Black" = "#B07AA1",
                               "Other" = "#3CB371")) +
  theme_minimal() +
...
```{r cluster_data}
cluster_data <- duke_nicu_clean_2 %>%
  select(gestational_age, birth_weight,
         ivh_2, rop_2, nec_2, pda_2) %>%
  mutate(across(ends_with("_2"), ~ ifelse(. == "Yes", 1, 0))) %>%
  scale()
```
...
fviz_nbclust(cluster_data, kmeans, method = "wss") +
  labs(x = "Number of Clusters (k)",
       ...
       title = "Elbow Plot for Determining the Number of Clusters")
```

```{r kmeans}
set.seed(1234)

kmeans_k2 <- kmeans(cluster_data, centers = 2, nstart = 20)
kmeans_k4 <- kmeans(cluster_data, centers = 4, nstart = 20)
kmeans_k5 <- kmeans(cluster_data, centers = 5, nstart = 20)
kmeans_k7 <- kmeans(cluster_data, centers = 7, nstart = 20)
```

```{r kmeans$cluster}
duke_nicu_clean_2 <- duke_nicu_clean_2 %>%
  mutate(cluster_2 = factor(kmeans_k2$cluster),
         cluster_4 = factor(kmeans_k4$cluster),
         cluster_5 = factor(kmeans_k5$cluster),
         cluster_7 = factor(kmeans_k7$cluster))
```

```{r cluster_summary-function}
cluster_summary <- function(data, cluster_var) {data %>%
    group_by({{cluster_var}}) %>%
    summarize(mean_ga = mean(gestational_age, na.rm = TRUE),
              mean_bw = mean(birth_weight, na.rm = TRUE),
              ivh_rate = mean(ivh_2 == "Yes", na.rm = TRUE),
              rop_rate = mean(rop_2 == "Yes", na.rm = TRUE),
              nec_rate = mean(nec_2 == "Yes", na.rm = TRUE),
              pda_rate = mean(pda_2 == "Yes", na.rm = TRUE),
              mean_los = mean(length_of_stay, na.rm = TRUE))}
```

```{r cluster_summary}
summary_k2 <- cluster_summary(duke_nicu_clean_2, cluster_2)
summary_k4 <- cluster_summary(duke_nicu_clean_2, cluster_4)
summary_k5 <- cluster_summary(duke_nicu_clean_2, cluster_5)
summary_k7 <- cluster_summary(duke_nicu_clean_2, cluster_7)
...
```{r fviz_cluster}
fviz_cluster(kmeans_k2, data = cluster_data, geom = "point") +
  ggtitle("K-Means Clustering (k = 2)")

fviz_cluster(kmeans_k4, data = cluster_data, geom = "point") +
  ggtitle("K-Means Clustering (k = 4)")

fviz_cluster(kmeans_k5, data = cluster_data, geom = "point") +
  ggtitle("K-Means Clustering (k = 5)")

fviz_cluster(kmeans_k7, data = cluster_data, geom = "point") +
  ggtitle("K-Means Clustering (k = 7)")
```

```{r fviz_cluster-kmeans_k4}
fviz_cluster(kmeans_k4, data = cluster_data, geom = "point") +
  labs(x = "Principal Component 1",
       y = "Principal Component 2",
       title = "K-Means Clustering of Infant Groups (k = 4)",
       color = "Infant Group",
       shape = "Infant Group",
       fill = "Infant Group") +
  scale_color_discrete(breaks = c("4", "1", "3", "2"),
                       labels = c("Extremely Premature",
                                  "Very Premature",
                                  "Moderately Premature",
                                  "Near-Term")) +
  scale_fill_discrete(breaks = c("4", "1", "3", "2"),
                      labels = c("Extremely Premature",
                                 "Very Premature",
                                 "Moderately Premature",
                                 "Near-Term")) +
  scale_shape_discrete(breaks = c("4", "1", "3", "2"),
                       labels = c("Extremely Premature",
                                  "Very Premature",
                                  "Moderately Premature",
                                  "Near-Term")) +
...
```{r group_4}
duke_nicu_clean_2 <- duke_nicu_clean_2 %>%
  mutate(group_4 = factor(cluster_4,
                          levels = c("4", "1", "3", "2"),
                          labels = c("Extremely Premature",
                                     "Very Premature",
                                     "Moderately Premature",
...
cluster_summary(duke_nicu_clean_2, group_4)
```

```{r cluster4_summary}
cluster4_summary <- duke_nicu_clean_2 %>%
  group_by(group_4) %>%
  summarize(mean_ga = mean(gestational_age, na.rm = TRUE),
            mean_bw = mean(birth_weight, na.rm = TRUE),
            ivh_rate = mean(ivh_2 == "Yes", na.rm = TRUE),
            ...
            pda_rate = mean(pda_2 == "Yes", na.rm = TRUE),
            mean_los = mean(length_of_stay, na.rm = TRUE)) %>%
  pivot_longer(-group_4, names_to = "measure", values_to = "value") %>%
  mutate(measure = factor(measure,
                          levels = c("mean_ga",
                                     ...
                                     "pda_rate",
                                     "mean_los"),
                          labels = c("Mean Gestational Age (Weeks)",
...
```{r cluster4_summary-col}
ggplot(data = cluster4_summary,
       mapping = aes(x = group_4,
                     y = value)) +
  ...
  labs(x = "Infant Group",
       y = NULL,
       title = "Clinical Profiles of Infant Groups (k = 4)")
```
...
```{r predictor_formula-predictor_formula_log}
predictor_formula <- length_of_stay ~ survival_status + gestational_age + birth_weight + gender + inborn + race_ethnicity + ivh + rop + nec + pda

predictor_formula_log <- log_los ~ survival_status + gestational_age + birth_weight + gender + inborn + race_ethnicity + ivh + rop + nec + pda

predictor_formula_binary <- length_of_stay ~ survival_status + gestational_age + birth_weight + gender + inborn + race_ethnicity + ivh_2 + rop_2 + nec_2 + pda_2

predictor_formula_log_binary <- log_los ~ survival_status + gestational_age + birth_weight + gender + inborn + race_ethnicity + ivh_2 + rop_2 + nec_2 + pda_2
```
...
```{r full_lm_log}
full_lm_log <- lm(predictor_formula_log,
                  data = train_data_2)
...
### Log-Linear Regression Model (Categorical Morbidities)

```{r full_lm_log_categorical}
full_lm_log_categorical <- lm(predictor_formula_log_binary,
                              data = train_data_2)
...
### Cross-Validation for Full, Log, and Categorical Linear Regression Models
...
```{r lm_caret}
lm_caret <- train(predictor_formula,
                  data = train_data,
                  method = "lm",
                  trControl = ctrl,
                  metric = "RMSE")
...
```{r lm_log_caret}
lm_log_caret <- train(predictor_formula_log,
                      data = train_data_2,
                      method = "lm",
                      trControl = ctrl,
                      metric = "RMSE")
...
lm_log_categorical_caret <- train(predictor_formula_log_binary,
                                  data = train_data_2,
                                  method = "lm",
                                  trControl = ctrl,
                                  metric = "RMSE")
...
### Residuals vs. Fitted Plot for Full and Log Linear Regression Models
...
### Normal Q-Q Plot for Log-Linear Regression Model
...
```{r tree_model}
tree_model <- rpart(predictor_formula_log,
                    data = train_data_2,
                    method = "anova")
...
```{r rf_model}
rf_model <- ranger(formula = predictor_formula,
                   data = train_data,
                   num.trees = 500,
                   importance = "impurity")
...
```{r rf_log_model}
rf_log_model <- ranger(formula = predictor_formula_log,
                       data = train_data_2,
                       num.trees = 500,
                       importance = "impurity")
...
### Random Forest Model (Categorical Morbidities)

```{r rf_categorical_model}
rf_categorical_model <- ranger(formula = predictor_formula_binary,
                               data = train_data,
                               num.trees = 500,
                               importance = "impurity")
...
### Log-Random Forest Model (Categorical Morbidities)

```{r rf_categorical_log_model}
rf_categorical_log_model <- ranger(formula = predictor_formula_log_binary,
                                   data = train_data_2,
                                   num.trees = 500,
                                   importance = "impurity")
...
### Random Forest Comparison (Using OOB RMSE)

```{r rf_compare}
tibble(model = c("Full Random Forest",
                 "Log-Random Forest",
                 "Random Forest (Categorical)",
                 "Log-Random Forest (Categorical)"),
...
### Grid Search for mtry and Number of Trees
...
### Variable Importance in the Final Random Forest Model
...
### 90th Percentile Restriction Split
...
```{r train_data_90_log, test_data_90_log}
train_data_90_log <- train_data_90 %>%
  mutate(log_los = log(length_of_stay + 1))

test_data_90_log <- test_data_90 %>%
  mutate(log_los = log(length_of_stay + 1))
```


### Log-Linear Regression with Restriction to the 90th Percentile Model

```{r full_lm_log_90}
full_lm_log_90 <- lm(predictor_formula_log,
                     data = train_data_90_log)

summary(full_lm_log_90)
```


### Final Random Forest with Restriction to the 90th Percentile Model
...
```{r lm_log_preds, lm_log_preds_days, lm_log_preds_90, lm_log_preds_90_days, rf_preds, rf_preds_90}
lm_log_preds <- predict(full_lm_log, newdata = test_data_2)
lm_log_preds_days <- exp(lm_log_preds) - 1

lm_log_preds_90 <- predict(full_lm_log_90, newdata = test_data_90_log)
lm_log_preds_90_days <- exp(lm_log_preds_90) - 1
...
```{r rmse_comparison}
rmse_comparison <- tibble(Model = c("Log-Linear (Full Test)",
                                    "Log-Linear (90th Percentile)",
                                    "Random Forest (Full Test)",
                                    "Random Forest (90th Percentile)"),
                          RMSE = c(rmse(test_data$length_of_stay, lm_log_preds_days),
                                   rmse(test_data_90$length_of_stay, lm_log_preds_90_days),
